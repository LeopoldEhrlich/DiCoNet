...
class SplitLayer(nn.Module):
    ...
    def forward(self, input_n, hidden, phi, nh):
        ...
        # h_conv has shape (batch_size, (times?) n, hidden_size + input_size)
...
class Split(nn.Module):
    ...
    # e is the group number over each input
    def forward(self, e, input, mask, scale=0):
        # Previous hidden output, since there is none to start, init it to random numbers
        hidden = Variable(torch.randn(self.batch_size, self.n,
                                      self.hidden_size)).type(dtype)
        
        # If at the top level, everything in one group
        if scale == 0:
            e = Variable(torch.zeros(self.batch_size, self.n)).type(dtype)

        # Gets the mask of the splits to date
        Phi = self.build_Phi(e, mask)

        # Term used to average kernel function
        ...
        # Final hidden state with each element of the input seen independently
        hidden_p = hidden.view(self.batch_size * self.n, self.hidden_size)

        # Run final linear layer and sigmoid
        scores = self.linear_b(hidden_p)
        probs = torch.sigmoid(scores).view(self.batch_size, self.n) * mask # Goes back to batches so probs has shape (batch_size, n)
        
        return scores, probs, input_n, Phi

    # For the given subset to split (masked region over X), phi is the matrix that represents each group
    # Each square in phi is a split area, can use to apply operations only within their groups
    ...
    # Normalizes each input using phi over all x to get the specific area of intrest
